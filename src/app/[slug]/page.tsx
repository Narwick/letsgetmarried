...
export default async function PublicSite({
  ...
  const [{ data: giftsData }, { data: rsvpData }] = await Promise.all([
    ...
    <div className="scroll-smooth bg-background text-foreground" style={themeStyle(wedding.theme, wedding.custom_accent)}>
      ...
      {hasPix && (regularGifts.length > 0 || honeymoon.length > 0) && (
        <section id="presentes" className="scroll-mt-16 px-6 py-20 text-[var(--on-ink)]" style={inkBg}>
          ...
          {regularGifts.length > 0 && (
            <GiftGroup title="Para o nosso lar" gifts={regularGifts} pixFor={pixFor} />
          )}
          {honeymoon.length > 0 && (
            <GiftGroup title="Para a nossa lua de mel ✈️" gifts={honeymoon} pixFor={pixFor} />
          )}
        </section>
      )}
